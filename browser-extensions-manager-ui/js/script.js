...
function fetch_data_and_create_cards() {
  fetch("../js/data.json")
    ...
    .then((json_data) => {
      const extension_card_section = document.querySelector(
      ...
      json_data.forEach((each_data) => {
        ...
        extension_card_header_section.classList.add(
          "extension-card-header-section"
        );
        ...
        const extension_card_activation_section =
          document.createElement("section");
        extension_card_activation_section.classList.add(
          "extension-card-activation-section"
        );
...
function removing_box_shadows_from_extension_list_items() {
  const extension_list_items = document.querySelectorAll(
    ".extension-buttons-list-item"
  );
  ...
  if (body_element.classList.contains("dark-theme")) {
    ...
    for (let i = 0; i < extension_list_items.length; i++) {
      extension_list_items[i].style.boxShadow =
        "var(--CLR-EXTENSION-BUTTON-BOX-SHADOW)";
    }
  }
}
...
const buttons_list_section = document.querySelector(
  ".extension-buttons-section"
);

buttons_list_section.addEventListener(
  "click",
  extension_buttons_list_clicked,
  false
);
...
function extension_buttons_list_clicked(event) {
  ...
  const button_list_items = document.querySelectorAll(
    ".extension-buttons-list-item"
  );
...
function set_elements(all_active_inactive) {
  const cards_activate_buttons = document.querySelectorAll(
    ".extension-card-activate-button"
  );
...
function extension_card_clicked(event) {
  ...
  const is_activation_button_clicked =
    check_activation_button_clicked(target_element);

  if (is_activation_button_clicked === true) return;

  const is_remove_button_clicked = check_remove_button_clicked(target_element);

  if (is_remove_button_clicked === true) return;
}
